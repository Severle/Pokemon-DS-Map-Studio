...
public class ColorWheelPanel extends JPanel {
    ...
    private void formMouseDragged(MouseEvent evt) {
        ...
        int distToCenter = (int) Math.sqrt((x - (double) size / 2) * (x - (double) size / 2) + (y - (double) size / 2) * (y - (double) size / 2));
        if (distToCenter < size / 2) {
            currentX = x;
            currentY = y;
        } else {
            float angle = (float) Math.atan2(x - (double) size / 2, y - (double) size / 2);
            currentX = (int) (((double) size / 2) * Math.sin(angle)) + size / 2;
            currentY = (int) (((double) size / 2) * Math.cos(angle)) + size / 2;
        }
    ...
    private void formMousePressed(MouseEvent evt) {
        ...
        if (distToCenter < size / 2) {
            ...
            float angle = (float) Math.atan2(x - (double) size / 2, y - (double) size / 2);
            currentX = (int) (((double) size / 2) * Math.cos(angle)) + size / 2;
            currentY = (int) (((double) size / 2) * Math.sin(angle)) + size / 2;
        }
    ...
    private BufferedImage generateColorWheel(int diameter, float brightness) {
        ...
        for (int i = 0; i < img.getWidth(); i++) {
            for (int j = 0; j < img.getHeight(); j++) {
                Color c;
                int distToCenter = (int) Math.sqrt((i - (double) img.getWidth() / 2) * (i - (double) img.getWidth() / 2) + (j - (double) img.getHeight() / 2) * (j - (double) img.getHeight() / 2));
                if (distToCenter < radius) {
                    float h = (float) (Math.atan2(i - (double) img.getWidth() / 2, j - (double) img.getHeight() / 2) / (2 * Math.PI));
    ...
    public Color getSelectedColor() {
        int distToCenter = (int) Math.sqrt((currentX - (double) size / 2) * (currentX - (double) size / 2) + (currentY - (double) size / 2) * (currentY - (double) size / 2));
        float h = (float) (Math.atan2(currentX - (double) size / 2, currentY - (double) size / 2) / (2 * Math.PI));
        float s = (float) distToCenter / ((float) size / 2);
    ...
    public void setHueSaturarion(float hue, float saturation) {
        ...
        float distToCenter = saturation * ((float) size / 2);
    ...
    public void setColor(Color c) {
        ...
        float distToCenter = hsb[1] * ((float) size / 2);
