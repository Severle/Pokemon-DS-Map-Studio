...
public class BdhcWriterHGSS {
    ...
    private static void writeStripes(BinaryBufferWriter writer, ArrayList<Stripe> stripes) {
        int offset = 0;
        for (Stripe stripe : stripes) {
    ...
    private static void writePlates(BinaryBufferWriter writer,
                                    int[][] pointIndices, int[] slopeIndices, int[] zIndices) {
    ...
    public static int getNumStripes(ArrayList<ArrayList<Stripe>> stripes) {
        return BdhcWriterDP.getNumStripes(stripes);
    }

    public static int getNumPlateIndices(ArrayList<Stripe> stripes) {
        int numIndices = 0;
        for (Stripe stripe : stripes) {
            numIndices += stripe.plateIndices.size();
        }
        return numIndices;
    }
